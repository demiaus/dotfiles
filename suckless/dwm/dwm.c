...
struct Client {
    ...
    int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, notallowed, isterminal, noswallow;
    pid_t pid;
    Client *next;
    Client *snext;
    Client *swallowing;
...
tile(Monitor *m)
{
    ...
    for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if (i < m->nmaster) {
            ...
            resize(c, m->rmaster ? m->wx + m->ww - mw
                                 : m->wx, m->wy + my, mw - 2*bw, h - 2*bw, bw, 0);
            if (my +  HEIGHT(c) < m->wh)
                my += HEIGHT(c);
        } else {
            h = (m->wh - ty) / (n - i);
            resize(c, m->rmaster ? m->wx
                                 : m->wx + mw, m->wy + ty, m->ww - mw - 2*bw, h - 2*bw, bw, 0);
            if (ty +  HEIGHT(c) < m->wh)
                ty += HEIGHT(c);
        }
}
